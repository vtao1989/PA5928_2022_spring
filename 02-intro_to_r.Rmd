...
A variable is used to store data including value, vector, data frame, etc., which R could use to manipulate [@tutorialspoint2]. 
...
It is important to clearly know the type of the variable since different types of variables may have different functions or operations to deal with. Another caveat is that the outlook of the variable may not show its real variable type. For example, 
...
```{r}
c <- c(5, 'good') # if you create a vector containing mixed variable types,
                  # such as numeric and character, R will restrict them to
                  # be the same variable type, here, character
c
```
...
Data frame is a **frequently-used** data structure in R. It could include columns with different types of values. Let's create a data frame with mixed variables types using ``data.frame()`` function.
...
We created a data frame storing the students' ID, name, and their test scores. We can select elements from this data frame with several ways.
...
Factor is the nominal variable in R. This type is very useful when we want to analyze data from different groups, such as gender, school, etc.
