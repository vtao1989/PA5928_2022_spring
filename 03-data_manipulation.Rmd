...
We will introduce how to manipulate with different datasets using base (i.e., inbuilt) functions in R.

## Import and save datasets

There are couple of ways to importing and saving different types of datasets [@quickr1;@quickr2].
...
mydata <- read.csv('c:/minneapolis.csv', ## file location and name
                   header = TRUE, ## read the first line as variable names
...
Besides importing data by codes, you could also import data with the system tool. If this is your first time to use this tool, there may be a process to install the packages. But don't worry, RStudio can do it by itself. You just need to click the button to approve the installation.
...
Usually, it takes less time to save file in CSV and CSV file has a smaller size in storage, compared with other file types. Note that if you do not specify file location when saving it, R will save it to the working folder.

### File location

As you can see in the examples, you need to specify the location (or path) of the file to make sure that R could find your file in the right position. Usually you could find the location by checking the system property of the file. In window system, you need to use forward slash ``/`` to separate the locations. However, in Mac system, you use backslash ``\``.

Inputting file location could be tedious, but you could avoid this. Before that, let's first learn a concept called working directory.  It is the location of the folder you are currently working in.  You can find your current working directory in the title region of the Console pane. Or you can use ``getwd()`` to find it. Importing data file from current working directory does not need to indicate file location in the import functions. Putting your datasets in your working directory can save you some codes.

If you open RStudio by double-clicking the R files, RStudio will use the file location of the R files as current working directory.  With this RStudio feature, we can avoid specifying the location of the data file with two steps. First, put your R file and dataset in the same folder. Then open RStudio by double clicking the R file. This is highly recommended. It would be easier for others to check your codes as they do not need to change the path of the file.

If you open RStudio directly, the current working directory is usually the default folder by the system. If you want to change your working directory, you can use ``setwd()`` and input the location of your preferred folder in the function (quotation marks are needed).
